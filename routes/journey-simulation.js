...
router.post('/simulate-journey', async (req, res) => {
  ...
  // Restart auto-load watcher if it was stopped (e.g. by a stop-everything call)
  // startAutoLoadWatcher is idempotent — does nothing if already running
  if (typeof global.startAutoLoadWatcher === 'function') {
    global.startAutoLoadWatcher();
  }
  ...
  try {
    ...
    // handled by the feature flag system in dynamic-step-service.cjs.
    ...
    const errorPlannedSteps = stepData.map(s => {
      ...
      if (hasErrorFromJourneyData) {
        ...
        // Check if this service has active chaos (feature flags with errors_per_transaction > 0)
        // Resolve the actual service name the same way service-manager does
        const svcFlags = global.serviceFeatureFlags || {};
        const companyClean = (currentPayload.companyName || '').replace(/[^a-zA-Z0-9]/g, '');
        
        // Build candidate service names to check against chaos flags
        const baseSvcName = s.serviceName || `${s.stepName}Service`;
        const candidateKeys = [
          `${baseSvcName}-${companyClean}`,
          `${s.stepName}Service-${companyClean}`,
          `${s.stepName}-${companyClean}`,
          baseSvcName,
          s.stepName
        ];
        ...
        for (const key of candidateKeys) {
          if (svcFlags[key] && svcFlags[key].errors_per_transaction > 0) {
            chaosFlags = svcFlags[key];
            console.log(`[journey-sim] 🎯 Chaos flag match: step=${s.stepName} matched key=${key}`);
            break;
          }
        }
        
        // Fuzzy: check if any key for this company contains context matching this step
        if (!chaosFlags) {
          const stepLower = s.stepName.toLowerCase();
          for (const [key, flags] of Object.entries(svcFlags)) {
            if (key.toLowerCase().endsWith(`-${companyClean.toLowerCase()}`) && flags.errors_per_transaction > 0) {
              const svcPart = key.split('-').slice(0, -1).join('-').toLowerCase().replace('service', '');
              if (svcPart.includes(stepLower) || stepLower.includes(svcPart)) {
                chaosFlags = flags;
                console.log(`[journey-sim] 🎯 Chaos flag fuzzy match: step=${s.stepName} matched key=${key}`);
                break;
              }
            }
          }
        ...
        const chaosRate = chaosFlags?.errors_per_transaction || 0;
        
        if (chaosRate > 0 && Math.random() < chaosRate) {
          const errorType = chaosFlags?.error_type || 'service_unavailable';
          const httpStatus = chaosFlags?.http_error_code || 500;
          ...
          return {
            ...
            errorType,
            httpStatus,
            errorMessage: generateErrorMessage(errorType, s.stepName),
            retryable: ![400, 404, 422].includes(Number(httpStatus)),
            severity: Number(httpStatus) >= 500 ? 'critical' : Number(httpStatus) >= 400 ? 'warning' : 'info'
          };
        }
        
        console.log(`[journey-sim] ✅ Success configured for step: ${s.stepName} (from Step 3 journey processing${chaosRate > 0 ? `, chaos=${chaosRate} but passed this time` : ''})`);
        return { ...s, hasError: false };
      }
    });
    ...
    // NOTE: Auto-start of LoadRunner disabled to prevent infinite respawn loop.
    // LoadRunner was auto-spawning on every journey sim, which generated more journeys,
    // which spawned more LoadRunners. Use the UI or API to start LoadRunner explicitly.
    if (!isFromLoadRunner) {
      console.log(`[journey-sim] ℹ️  LoadRunner auto-start disabled (use UI/API to start explicitly)`);
      continuousGeneration = { active: false, reason: 'auto_start_disabled' };
    } else {
...
router.post('/simulate-multiple-journeys', async (req, res) => {
  ...
  try {
    ...
    // v2.6.6: Force-disable legacy error simulation
    const errorSimulationEnabled = false;
