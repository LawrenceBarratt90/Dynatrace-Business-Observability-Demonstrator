...
function createStepService(serviceName, stepName) {
  ...
  createService(properServiceName, (app) => {
    ...
    app.post('/process', async (req, res, next) => {
      ...
      try {
        // Nemesis-targeted chaos should fail inside this runtime service so LiveDebugger
        // can capture the intended error injection branch, not short-circuit beforehand.
        const deferExplicitStepError = payload.chaosManagedByService === true;
        if (deferExplicitStepError && payload.hasError === true) {
          console.log(`[${properServiceName}] Deferring explicit step error to runtime chaos injection path`);
        }

        // Check for non-Nemesis step errors first (both explicit and simulated)
        const stepError = deferExplicitStepError ? null : checkForStepError(payload, null); // You can pass error profile here
        if (stepError) {
