...
const DEMO_PATHS: DemoPath[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: 'Select instances by label (recommended)',
        action: 'In "Customize your debug session", use Labels instead of manually selecting host/process. Add filter `deployment.release_product` and select `BizObs-Demonstrator` (or the product label used by your environment). This auto-selects the generated journey services for the session.',
        where: 'Dynatrace LiveDebugger',
        tip: 'Label-based selection is stable across restarts and new PIDs, so you do not need to re-pick processes each run.',
      },
      {
        title: 'Repository setup workaround (if top repo shows Failed to fetch)',
        action: 'If the top repository entry cannot be removed, add the same repo manually and continue with the fetched/manual entry. If LiveDebugger keeps showing the failed one, start a new debug session and add only the manual/fetched repository before setting breakpoints.',
        where: 'Dynatrace LiveDebugger',
        tip: 'A stale auto-detected repo entry can appear as locked. It does not block debugging as long as one repository entry is successfully fetched and selected.',
      },
      {
      ...
      {
        title: 'Breakpoint 1 — Line 696/697: Nemesis error gate + payload',
        action: 'Set a breakpoint on line 696 (`if (shouldError)`) or line 697 (`errorInjected = { ... }`). These lines fire when Nemesis/feature-flag chaos actually injects an error. They capture the raw injected fault before the error object is finalized.',
        where: 'Dynatrace LiveDebugger',
        tip: 'Use 696 for the decision gate and 697 for the full injected payload. You will see `errorInjected` (error_type, http_status, feature_flag, remediation_action), plus `errorConfig` and journey/customer context.',
      },
      {
        title: 'Breakpoint 2 — Line 708: Error emitted to runtime',
        action: 'Set a second breakpoint on line 708 (`console.log(...)` after the injected error object is assembled). This gives a clean capture point for the error details and correlation context used in the failure path.',
        where: 'Dynatrace LiveDebugger',
        tip: 'Important: do not set breakpoints on lines 706 or 707. Those are not valid Node.js breakpoint locations in this file and will stay Pending with LiveDebuggerInvalidBreakpointLocation.',
      },
      {
      ...
      },
      {
        ...
        action: 'Right-click any breakpoint to add a condition. Useful conditions:\n\n• Line 696/697/708: errorInjected !== null — only fire when chaos is actually injected\n• Line 696/697/708: errorInjected.http_status === 500 — catch only server errors\n• Line 996: nextServiceName && nextServiceName.toLowerCase().includes("payment") — focus on payment chain hops',
        where: 'Dynatrace LiveDebugger',
        tip: 'Conditions keep snapshots focused and prevent noise. For Nemesis demos, 696 + `errorInjected !== null` is the cleanest trigger.',
      },
      {
        ...
        where: 'Dynatrace LiveDebugger',
        tip: 'If snapshots are Pending: (1) confirm service traffic is active, (2) ensure Chaos/Nemesis is injecting errors, (3) verify breakpoint lines are valid (use 696/697, 708, 996; avoid 706/707).',
      },
      {
        title: 'Inspect snapshot data',
        action: 'Click a snapshot to expand the variable state captured at that moment:\n\n• Line 696/697 snapshot — `errorInjected` ({ error_type, http_status, feature_flag, remediation_action }), plus `errorConfig` and request context\n• Line 708 snapshot — injected error details and immediate runtime context in the failure branch\n• Line 996 snapshot — `nextPayload`, `nextServiceName`, and trace headers used for service chaining',
        where: 'Dynatrace LiveDebugger',
        tip: 'Use these three breakpoints together: injection decision (696/697), failure path context (708), and downstream propagation (996).',
      },
      {
...
const PERSONAS: Persona[] = [
  ...
  },
  {
    ...
    demoFlow: [
      ...
      { step: '8. LiveDebugger deep-dive (optional, 2 min)', detail: 'Open LiveDebugger and select instances using the `deployment.release_product` label (for example `BizObs-Demonstrator`) instead of manually selecting host/process. Set breakpoints on lines 696 or 697 (Nemesis error injection), 708 (failure-path context), and 996 (service chain call) in dynamic-step-service.js. Avoid lines 706/707 (invalid locations). Capture snapshots with injected fault fields, correlation context, and chain payloads.', dtLink: { label: 'LiveDebugger', url: `${TENANT_URL}/ui/apps/dynatrace.devobs.debugger/debugger` } },
