...
The Engine runs on your host (EC2, VM, Codespace). The Demonstrator UI runs inside Dynatrace and talks to the Engine through an **EdgeConnect** tunnel.
...
This is a **single unified repo** — it contains both the Engine (server) and the Demonstrator UI (AppEngine app).
...
| **Demonstrator Dashboards** | `/forge-dashboards` | DQL-powered dashboard presets (Security, DI, Infra, etc.) + Librarian modal overlay for AI-driven incident analysis |
