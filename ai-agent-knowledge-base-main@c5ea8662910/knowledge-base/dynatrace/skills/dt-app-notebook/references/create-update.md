...
1. **Write query in .dql file (NOT in notebook yet)**
   ...
   fetch spans
   | filter dt.entity.service == "SERVICE-REAL-ID"
   | summarize count()
   EOF
   ```
...
| summarize error_count = count(), unique_messages = countDistinct(content), by: {dt.entity.service, loglevel}
...
```json
{
  "sections": [
    "... existing sections ...",
    {
      ...
      "state": {
        "input": {
          "value": "fetch logs | filter loglevel == \"ERROR\" | summarize count(), by: {dt.entity.service, dt.entity.host} | sort count desc | limit 20",
          "timeframe": {
            "from": "now()-2h",
            "to": "now()"
          }
