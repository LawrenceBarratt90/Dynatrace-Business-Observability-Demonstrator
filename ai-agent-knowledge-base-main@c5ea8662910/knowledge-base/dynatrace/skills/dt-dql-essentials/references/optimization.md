...
| summarize count(), by: {process_group}
```
...
```dql
fetch logs, from:now()-1h
| summarize count(), by: {process_group, severity}
| filter severity == "ERROR"
```
...
```dql
fetch logs, from:now()-1h
| summarize count(), by: {process_group = entityName(dt.entity.process_group)}
```
...
| summarize
    ...
    warnings = countIf(severity == "WARN")
```
...
fetch logs | filter severity == "ERROR" | summarize errors = count()
fetch logs | filter severity == "WARN" | summarize warnings = count()
```
...
| filter severity == "ERROR"                           // 2. Filter early
| fieldsAdd process_group = entityName(dt.entity.process_group)  // 3. Add fields
| summarize error_count = count(), by: {process_group} // 4. Aggregate
...
```dql
fetch logs
| filter severity == "ERROR" and http.status_code == 500
```
...
```dql
fetch logs
| filter in(severity, "ERROR", "FATAL", "WARN")
```
...
```dql
fetch logs
| filter severity == "ERROR" or severity == "FATAL" or severity == "WARN"
```
...
fetch logs
| filter severity == "ERROR"
| fieldsAdd process = entityName(dt.entity.process_group)
```
...
More efficient for conditional counts:
...
fetch logs
| filter severity == "ERROR"
| summarize error_count = count(), by: {severity}
...
fetch logs
| filter severity == "ERROR"
| fields timestamp, content, severity, http.status_code
| summarize count(), by: {severity, http.status_code}
```
...
| summarize count(), by: {severity, http.status_code}
| fields timestamp, content, severity, http.status_code
// Fields command after summarize has no effect
```
...
```dql
fetch logs
| fields timestamp, level = severity, message = content
```
...
```dql
fetch logs
| fieldsAdd level = severity, message = content
...
fetch logs, from:now()-24h
| fieldsAdd process = entityName(dt.entity.process_group)
| summarize count(), by: {severity, process}
| filter severity == "ERROR"
```
...
```dql
fetch logs, from:now()-1h
| summarize count(), by: {service = entityName(dt.entity.service)}
```
...
| summarize
    ...
    by: {bin(timestamp, 5m), service = entityName(dt.entity.service)}
...
```dql
fetch logs
| filter severity == "ERROR" and http.status_code >= 500 and response_time > 1000
```
