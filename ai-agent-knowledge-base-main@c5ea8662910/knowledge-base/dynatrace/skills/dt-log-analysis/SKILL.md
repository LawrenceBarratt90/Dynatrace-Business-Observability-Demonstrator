...
- **dt.entity.process_group**: Associated process group entity
- **entityName()**: Resolves entity IDs to human-readable names
...
| filter status == "ERROR"
| fields timestamp, content, process_group = entityName(dt.entity.process_group)
| sort timestamp desc
| limit 100
```
...
| fieldsAdd process_group = entityName(dt.entity.process_group)
| sort `count()` desc
```
...
| summarize
    ...
    by: {process_group = entityName(dt.entity.process_group)}
```
...
### Entity Operations
- `entityName(dt.entity.process_group)` - Get human-readable process group name
...
6. **Name entities** - Always use `entityName()` for human-readable output
7. **Use time buckets for trends** - `bin(timestamp, 5m)` for time-series analysis

## Integration Points

- **Entity model**: Uses `dt.entity.process_group` for service correlation
...
- **dql-essentials** - Core DQL syntax and query structure for log queries
- **application-tracing** - Correlate logs with distributed traces using trace IDs
- **problem-analysis** - Correlate logs with DAVIS-detected problems
