...
timeseries cpu_usage = avg(dt.process.cpu.usage),
    by: {dt.entity.process_group_instance, dt.entity.process_group}
...
timeseries memory_bytes = avg(dt.process.memory.working_set_size),
    by: {dt.entity.process_group_instance}, interval: 15m
| fieldsAdd process_name = entityName(dt.entity.process_group_instance, type:"dt.entity.process_group_instance")
| summarize
    first_value = takeFirst(memory_bytes),
    last_value = takeLast(memory_bytes),
    by: {dt.entity.process_group_instance, process_name}
| fieldsAdd
...
timeseries io_total = avg(dt.process.io.bytes_total),
    by: {dt.entity.process_group_instance}, interval: 1m
| fieldsAdd process_name = entityName(dt.entity.process_group_instance, type:"dt.entity.process_group_instance")
| summarize
    avg_io = avg(io_total),
    max_io = max(io_total),
