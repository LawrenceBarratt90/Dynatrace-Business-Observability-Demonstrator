...
| fieldsAdd
    age_hours = duration(lifetime.start, now(), unit: "h"),
...
| fieldsAdd lifespan_minutes = duration(lifetime.start, lifetime.end, unit: "m")
| filter lifespan_minutes < 10
...
| lookup [
  ...
  ], sourceField:runs_on[dt.entity.host], lookupField:id
