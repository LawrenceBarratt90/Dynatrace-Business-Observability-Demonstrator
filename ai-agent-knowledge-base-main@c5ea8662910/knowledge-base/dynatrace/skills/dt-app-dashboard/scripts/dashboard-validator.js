 ...
 *   3. Return { success, data } on success or { success, error } on failure
 *
 ...
 * @param {number}  maxRecords - Cap on returned records (default 100; tile queries use 10)
...
async function resolveVariables(variables) {
  ...
  for (const variable of sorted) {
    ...
    if (type === 'csv') {
      ...
      try {
        const result = await executeQuery(substituted, 100, 7 * 24 * 60);
        if (result.success && result.data.records?.length > 0) {
          ...
          const values = records.map(r => r[firstField]).filter(Boolean);
          const selectedValues = variable.multiple === true
            ? values
 ...
 *
 * The check works by "claiming" result fields for each required slot in order.
 * Once a field is claimed by a slot, it is not available for subsequent slots.
 * This prevents a single numeric field from satisfying both "Values" and
 * "Gauge value" in the same visualization, for example.
 *
...
function checkVisualizationCompatibility(visualization, resultData) {
  ...
  if (!requirements) return []; // no requirements for this visualization

  const fieldTypes = extractFieldTypes(resultData);
  const errors = [];
  const claimedFields = new Set();

  for (const slot of requirements) {
    const isRequired = slot.required !== false;
    const acceptsAny = slot.types === 'any';

    // Find unclaimed fields that match this slot's type requirements
    const matchingFields = Object.entries(fieldTypes)
      .filter(([field, type]) => {
        if (claimedFields.has(field)) return false;
        if (acceptsAny) return true;
        if (slot.types.includes(type)) return true;
        // numericArray matches both 'numericArray' and 'array' slot types
        if (type === 'numericArray' && slot.types.includes('numericArray')) return true;
        return false;
      })
      .map(([field]) => field);

    const needed = slot.count === 'oneOrMore' ? 1 : 1;

    if (matchingFields.length < needed) {
      if (isRequired) {
        const typeLabel = acceptsAny ? 'any type' : slot.types.join(' | ');
        errors.push(
          `${slot.name}: requires ${slot.count === 'oneOrMore' ? 'at least one' : 'one'} field of type [${typeLabel}], but ${matchingFields.length === 0 ? 'none found' : 'not enough matching fields'} in query result. Available fields: ${Object.entries(fieldTypes).map(([f, t]) => `${f} (${t})`).join(', ') || 'none'}`
        );
      }
      // Skip claiming if no matches
      continue;
    }

    // Claim the minimum needed fields
    if (slot.count === 'one') {
      claimedFields.add(matchingFields[0]);
    }
    // For 'oneOrMore', we only claim one to leave others available
    // (the visualization will use all matching fields at render time)
    if (slot.count === 'oneOrMore') {
      claimedFields.add(matchingFields[0]);
    }
  }
...
async function validateTile(tileQuery, resolvedVars) {
  const substitutedQuery = substituteVariables(tileQuery.query, resolvedVars);
