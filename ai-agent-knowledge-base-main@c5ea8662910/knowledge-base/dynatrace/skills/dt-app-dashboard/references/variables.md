    ...
    "variables": [
      {
        ...
        "visible": true,
        "editable": true,
        "input": "fetch dt.entity.service | fields entity.name",
        "multiple": false
      }
    ],
    "tiles": {},
...
```dql
fetch dt.entity.service | fields entity.name | sort entity.name asc
```

❌ **Wrong (returns 2 fields):**

```dql
fetch dt.entity.service | fields id, entity.name | sort entity.name asc
...
```json
{
  ...
  "visible": true,
  "editable": true,
  "input": "fetch dt.entity.service | fields entity.name | sort entity.name asc",
  "multiple": false
}
```
...
```json
{
  ...
  "visible": false,
  "editable": true,
  "input": "fetch dt.entity.kubernetes_cluster | filter entity.name==$Cluster | fields id",
  "multiple": false
}
```
...
**Usage:** `fetch dt.entity.service | expand tags | filter
...
[
  {
    "key": "Cluster",
    "type": "query",
    "input": "fetch dt.entity.kubernetes_cluster | fields entity.name"
  },
  {
    "key": "Namespace",
    "type": "query",
...
```dql
summarize by: {bin(timestamp, duration(toLong($resolution:noquote),
unit:"m"))}
```
