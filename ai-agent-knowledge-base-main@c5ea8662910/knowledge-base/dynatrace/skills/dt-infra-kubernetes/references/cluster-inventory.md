...
smartscapeNodes K8S_POD, K8S_DEPLOYMENT, K8S_STATEFULSET, K8S_DAEMONSET,
  K8S_SERVICE
| summarize resource_count = count(),
  by: {k8s.cluster.name, k8s.namespace.name, type}
...
| lookup [
  ...
  ], sourceField: [k8s.cluster.name, k8s.namespace.name],
  lookupField: [k8s.cluster.name, k8s.namespace.name]
