...
Spans reference services via entity IDs. Enrich spans with service names using entity lookups:

```dql
fetch spans
| fieldsAdd entityName(dt.entity.service)
| summarize spans=count(), by: { dt.entity.service.name }
```

**Entity functions**:
- `entityName(dt.entity.service)` - Adds `dt.entity.service.name` field
- `entityAttr(dt.entity.service, "attribute_name")` - Access specific entity attributes
...
| fields span.name, duration, entityName(dt.entity.service)
```
...
| filter duration > 5s
| fieldsAdd service_name = entityName(dt.entity.service)
| fields trace.id, span.name, service_name, duration
| sort duration desc
| limit 50
```
...
| summarize
  ...
  by: { entityName(dt.entity.service) }
...
| fieldsAdd
    caller_service = entityName(dt.entity.service),
    called_service = server.address
| summarize
    call_count = count(),
    avg_duration = avg(duration),
    by: {caller_service, called_service}
...
| fieldsAdd entityName(dt.entity.service)
| summarize {
    services = arrayRemoveNulls(collectDistinct(dt.entity.service.name)),
    trace_root = takeMin(record(
...
| summarize {
    spans=count(),
    avg_duration=avg(duration)
  }, by: { dt.entity.service.name, db.system, db.namespace }
| sort spans desc
```
...
- **Use `entityName()`**: Simplest way to add service names
- **Prefer subqueries**: More readable than `classicEntitySelector()`
- **Cache entity info**: Store entity lookups in fields for reuse
...
- **[Entity Lookups](references/entity-lookups.md)** - Advanced entity selectors, infrastructure correlation, hardware analysis
...
- **entity-model** - Understanding Dynatrace entity relationships
