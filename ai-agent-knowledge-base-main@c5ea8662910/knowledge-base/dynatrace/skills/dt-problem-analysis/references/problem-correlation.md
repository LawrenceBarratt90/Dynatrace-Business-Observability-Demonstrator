...
When DAVIS detects a problem, use `affected_entity_ids` to query logs and telemetry from impacted entities. This correlation helps identify the specific error conditions, configuration changes, or resource constraints that triggered the problem.

## Key Correlation Fields

| Field | Description | Usage |
|-------|-------------|-------|
| `affected_entity_ids` | Array of entity IDs directly impacted | Use in subqueries to filter logs/metrics |
...
fetch dt.davis.problems
| filter display_id == "P-12345678"
| fields problem_start=event.start, affected_entity_ids
| join [
    fetch logs
    | filter loglevel in ["ERROR", "WARN"]
], on:{left[affected_entity_ids] == right[dt.source_entity]}
...
fetch dt.davis.problems
| filter display_id == "P-12345678"
| fields problem_start = event.start, problem_entities = affected_entity_ids
...
fetch dt.davis.problems, from:now() - 2h
| filter not(dt.davis.is_duplicate) and event.status == "ACTIVE"
| fields problem_start = event.start, display_id, event.name, affected_entity_ids
| join [
    fetch events
    | filter event.type == "DEPLOYMENT"
], on:{left[affected_entity_ids] == right[dt.entity.service]}
| fieldsAdd time_since_deployment = problem_start - timestamp
| filter time_since_deployment > 0 and time_since_deployment < 30m
| fields display_id, event.name, deployment_name, time_since_deployment
```
...
| fields display_id, root_cause_entity_id, affected_entity_ids
...
1. **Identify problem**: Get `display_id` and `affected_entity_ids`
...
fetch dt.davis.problems
| fields affected_entity_ids  // Includes duplicates
```
...
3. **affected_entity_ids is an array**: Use `in` operator for filtering
...
fetch dt.davis.problems, from:now() - 1h
| filter not(dt.davis.is_duplicate) and event.status == "ACTIVE"
| fields display_id, event.start, dt.davis.event_ids, affected_entity_ids
| join [
    fetch dt.davis.events
