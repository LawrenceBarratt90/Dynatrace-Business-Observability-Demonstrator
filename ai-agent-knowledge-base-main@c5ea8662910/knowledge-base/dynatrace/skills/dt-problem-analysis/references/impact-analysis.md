...
- **Which services are impacted?** (`dt.entity.service`, `affected_entity_ids`)
...
| `affected_entity_ids` | Array of directly impacted entity IDs | array | Calculate blast radius |
| `affected_entity_types` | Types of affected entities | array | Understand scope |
| `dt.entity.service` | Affected service IDs | array | Business criticality |
...
| summarize
    ...
    total_affected_entities = sum(arraySize(affected_entity_ids)),
...
fetch dt.davis.problems, from:now() - 24h
| filter not(dt.davis.is_duplicate) and event.status == "ACTIVE"
| fieldsAdd service_count = arraySize(dt.entity.service)
| filter service_count > 1
| fields display_id, event.name, service_count, dt.entity.service
...
| filter dt.entity.service == "SERVICE-CRITICAL-APP"
| fields
    event.start,
    display_id,
    event.name,
...
| fieldsAdd downstream_services = arraySize(dt.entity.service)
...
| fieldsAdd
    ...
    entity_score = arraySize(affected_entity_ids) * 5,
...
| fieldsAdd
    ...
    is_wide_scope = arraySize(affected_entity_ids) > 10,
...
fetch dt.davis.problems
| filter arrayContains(dt.entity.service, "SERVICE-ABC")
...
fetch dt.davis.problems, from:now() - 2h
| filter not(dt.davis.is_duplicate) and event.status == "ACTIVE"
| filter dt.davis.affected_users_count > 10 or arraySize(affected_entity_ids) > 5
...
| fields display_id, dt.entity.service, affected_entity_ids, dt.davis.impact_level
```
...
| filter display_id == "P-XXXXXXXXXX"
| fieldsAdd
    total_entities = arraySize(affected_entity_ids),
    total_services = arraySize(dt.entity.service)
| fields display_id, total_entities, total_services, affected_entity_ids
```
