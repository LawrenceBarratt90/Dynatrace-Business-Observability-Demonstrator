...
| summarize
    ...
    by: {app_name, csp.violated_directive}
...
| summarize
    violation_count = count(),
    by: {app_name, csp.blocked_uri.full, csp.effective_directive}
| sort violation_count desc
| limit 30
...
| summarize
    violation_count = count(),
    by: {app_name, csp.blocked_uri.domain, csp.effective_directive}
| sort violation_count desc
| limit 20
...
| summarize
    violation_count = count(),
    by: {app_name, csp.effective_directive, csp.document_uri.path}
| sort violation_count desc
| limit 20
...
| summarize
    ...
    by: {app_name, csp.document_uri.path}
| sort violation_count desc
| limit 20
...
| summarize
    violation_count = count(),
    by: {app_name, time_bucket = bin(start_time, 1d)}
...
| summarize
    violation_count = count(),
    by: {app_name, csp.blocked_uri.domain, csp.document_uri.path}
| sort violation_count desc
| limit 20
...
| summarize
    ...
    affected_sessions = countDistinct(dt.rum.session.id),
    by: {app_name, csp.effective_directive}
