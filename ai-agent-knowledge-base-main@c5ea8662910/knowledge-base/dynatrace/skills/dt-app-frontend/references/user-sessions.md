...
| summarize
    ...
    by: {app_name, dt.rum.instance.id}
| summarize
...
| summarize
    ...
    by: {app_name, ui_element.resolved_name, ui_element.tag_name}
...
| filter characteristics.has_page_summary == true or characteristics.has_view_summary == true
| fieldsAdd app_name = entityName(dt.rum.application.entity, type:"dt.entity.application")
| summarize max_sequence = max(view.sequence_number), by: {app_name, dt.rum.session.id}
| summarize
    sessions = count(),
