...
| fieldsAdd service_name = entityName(dt.entity.service),
    meets_sla = if(request.is_failed == false AND duration < 3s, 1, else: 0)
| summarize total = count(), sla_compliant = sum(meets_sla), by: {service_name}
...
3. **Grouping** - Dimensions used (`dt.entity.service`, `k8s.workload.name`, etc.)
