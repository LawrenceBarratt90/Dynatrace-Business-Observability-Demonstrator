...
timeseries goroutine_count = avg(dt.runtime.go.scheduler.goroutine_count),
          by: {dt.entity.process, dt.entity.process_group, goroutine_owner = go.goroutine.owner},
          from: now() - 4h
| fieldsAdd
...
timeseries gc_suspension = avg(dt.runtime.go.gc.suspension_time),
          by: {dt.entity.process, dt.entity.process_group},
          from: now() - 1h
| fieldsAdd
...
timeseries heap_bytes = avg(dt.runtime.go.memory.heap),
          by: {dt.entity.process, dt.entity.process_group, heap_state = go.heap.state},
          from: now() - 2h
| fieldsAdd
...
timeseries memory_used = avg(dt.runtime.go.memory.used),
          by: {dt.entity.process, dt.entity.process_group, memory_type = go.memory_type},
          from: now() - 30m
| fieldsAdd
...
timeseries memory_limit = avg(dt.runtime.go.memory.limit),
          ...
          by: {dt.entity.process, memory_type = go.memory_type},
          from: now() - 2h
| fieldsAdd
...
timeseries worker_threads = avg(dt.runtime.go.scheduler.worker_thread_count),
          by: {dt.entity.process, dt.entity.process_group, thread_state = go.thread.state},
          from: now() - 1h
| fieldsAdd
...
timeseries request_rate = avg(dt.runtime.go.http.requests, rate:1s),
          by: {dt.entity.process, status_code = http.response.status_code},
          from: now() - 1h
