...
timeseries gc_count = avg(dt.runtime.jvm.gc.collection_count),
          ...
          by: {dt.entity.process, dt.entity.process_group, gc_type = jvm.gc.name},
          from: now() - 1h
| fieldsAdd
...
timeseries {
  ...
  by: {dt.entity.process, pool_name = jvm.memory.pool.name},
  from: now() - 1h
| fieldsAdd
...
timeseries {
  ...
  by: {dt.entity.process, gc_type = jvm.gc.name},
  from: now() - 30m
